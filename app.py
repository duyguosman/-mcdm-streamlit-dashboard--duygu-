...
from pymcdm.normalization import minmax_normalization, sum_normalization
...
try:
    ...
    if numeric_df.isnull().values.any():
        st.error("Matris geçersiz karakterler içeriyor!")
        st.stop()
...
weights_list, types_list = [], []

for col in criteria_names:
    st.sidebar.markdown(f"**{col}**")
    c1, c2 = st.sidebar.columns(2)
    with c1:
        weight = st.sidebar.slider(f"Weight", 0.0, 1.0, 1.0/len(criteria_names), key=f"w_{col}")
        weights_list.append(weight)
    with c2:
        ctype = st.sidebar.radio("Type", ["Benefit", "Cost"], key=f"t_{col}")
...
# --- 3. METHOD SELECTION ---
# MABAC'a normalizasyon fonksiyonunu açıkça veriyoruz
available_methods = {
    'TOPSIS': TOPSIS(),
    'SAW': SAW(normalization=minmax_normalization),
    'MABAC': MABAC(normalization=minmax_normalization), # BURASI KRİTİK
    'ARAS': ARAS(),
    'WSM': WSM(normalization=minmax_normalization)
}

selected_method_names = st.sidebar.multiselect(
    "Methods:", list(available_methods.keys()), default=['TOPSIS', 'SAW']
)

# --- 4. EXECUTE ---
if st.button("Run MCDM Analysis"):
    if not selected_method_names:
        st.warning("Lütfen bir yöntem seçin.")
    else:
        # 0 değerlerini minik bir sayıyla değiştir (MABAC'ın içindeki matematik için)
        clean_data = np.where(alts_data <= 0, 1e-6, alts_data)
        ...
        for name in selected_method_names:
            try:
                ...
                st.error(f"{name} hesaplanamadı: {e}")

        if successful_methods:
            col1, col2 = st.columns(2)
            with col1:
                st.subheader("Preference Table")
                st.dataframe(pd.DataFrame(zip(*prefs), columns=successful_methods, index=alts_names).round(4), use_container_width=True)
            with col2:
                st.subheader("Ranking Table")
                st.dataframe(pd.DataFrame(zip(*ranks), columns=successful_methods, index=alts_names).astype(int), use_container_width=True)
